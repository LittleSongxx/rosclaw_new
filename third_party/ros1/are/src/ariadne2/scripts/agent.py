...
class Agent:
    ...
    def _filter_neighbor_indices(self, all_key_node_coords, utility, current_index, neighbor_indices):
        if not parameter.WAVELET_NEIGHBOR_LIGHT_FILTER:
            return neighbor_indices
        if neighbor_indices is None or len(neighbor_indices) == 0:
            return neighbor_indices

        trigger_thresh = max(1, int(parameter.WAVELET_NEIGHBOR_FILTER_MIN_TRIGGER))
        if len(neighbor_indices) <= trigger_thresh:
            return neighbor_indices

        max_keep = max(1, int(parameter.WAVELET_NEIGHBOR_MAX_KEEP))
        min_keep = max(1, int(parameter.WAVELET_NEIGHBOR_MIN_KEEP))
        keep_n = min(len(neighbor_indices), max(max_keep, min_keep))
        if len(neighbor_indices) <= keep_n:
            return neighbor_indices

        wavelet_map = getattr(self.node_manager, "latest_wavelet_map", None)
        wavelet_map_info = getattr(self.node_manager, "latest_wavelet_map_info", None)
        if wavelet_map is None:
            return neighbor_indices
        if wavelet_map_info is None:
            wavelet_map_info = self.map_info

        candidate_coords = all_key_node_coords[neighbor_indices]
        wavelet_scores = np.array(
            [wavelet_scalar_at_coords(coords, wavelet_map_info, wavelet_map) for coords in candidate_coords],
            dtype=np.float32,
        )
        utility_scores = self._normalize_scores(np.asarray(utility[neighbor_indices], dtype=np.float32))
        distance = np.linalg.norm(candidate_coords - self.location.reshape(1, 2), axis=1).astype(np.float32)
        distance_scores = 1.0 - self._normalize_scores(distance)

        score = (
            float(parameter.WAVELET_NEIGHBOR_SCORE_WAVELET_WEIGHT) * wavelet_scores
            + float(parameter.WAVELET_NEIGHBOR_SCORE_UTILITY_WEIGHT) * utility_scores
            + float(parameter.WAVELET_NEIGHBOR_SCORE_DISTANCE_WEIGHT) * distance_scores
        )

        ranked_local = np.argsort(score)[::-1].tolist()
        current_local_index = np.where(neighbor_indices == current_index)[0]
        if current_local_index.size > 0:
            current_local = int(current_local_index[0])
            ranked_local = [current_local] + [idx for idx in ranked_local if idx != current_local]

        selected_local = np.array(ranked_local[:keep_n], dtype=np.int64)
        filtered = neighbor_indices[selected_local]
    ...
    def update_key_node_observation(self):
        ...
        current_index = np.argwhere(node_coords_to_check == self.location[0] + self.location[1] * 1j)[0][0]
        neighbor_indices = np.argwhere(adjacent_matrix[current_index] == 0).reshape(-1)
        neighbor_indices = self._filter_neighbor_indices(all_key_node_coords, utility, current_index, neighbor_indices)
