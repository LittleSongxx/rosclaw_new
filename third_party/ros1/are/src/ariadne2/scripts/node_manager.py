...
class NodeManager:
    def __init__(self, start=np.array([0,0])):
        ...
        self.latest_wavelet_map_info = None
        self._wavelet_cache_raw_map = None
    ...
    def _needs_wavelet_map(self):
        return parameter.WAVELET_ADAPTIVE_DTH or parameter.WAVELET_NEIGHBOR_LIGHT_FILTER

    def _build_wavelet_context(self, robot_location, map_info):
        if not self._needs_wavelet_map():
            self.latest_wavelet_map = None
            self.latest_wavelet_map_info = None
            return None, None

        wavelet_map_info = extract_local_map_info(
            robot_location,
            map_info,
            parameter.WAVELET_LOCAL_MAP_SIZE,
        )
        local_map = wavelet_map_info.map
        geom = (
            float(wavelet_map_info.map_origin_x),
            float(wavelet_map_info.map_origin_y),
            int(local_map.shape[0]),
            int(local_map.shape[1]),
            float(wavelet_map_info.cell_size),
        )

        reuse = False
        if (
            self.latest_wavelet_map is not None
            and self.latest_wavelet_map_info is not None
            and self._wavelet_cache_raw_map is not None
            and self._wavelet_cache_geom == geom
            and self._wavelet_cache_raw_map.shape == local_map.shape
        ):
            changed = np.count_nonzero(local_map != self._wavelet_cache_raw_map)
            change_ratio = float(changed) / float(local_map.size) if local_map.size > 0 else 1.0
            reuse = change_ratio <= float(parameter.WAVELET_CACHE_CHANGE_RATIO_THRESH)

        if not reuse:
            self.latest_wavelet_map = compute_wavelet_energy_map(local_map)
            self.latest_wavelet_map_info = wavelet_map_info
            self._wavelet_cache_raw_map = np.array(local_map, copy=True)
            self._wavelet_cache_geom = geom

        return self.latest_wavelet_map, self.latest_wavelet_map_info

    def adaptive_distance_threshold(self, ref_coords, map_info, wavelet_map, wavelet_map_info=None):
        base = parameter.THR_NEXT_WAYPOINT + parameter.NODE_RESOLUTION
        if not parameter.WAVELET_ADAPTIVE_DTH or wavelet_map is None:
            return base
        lookup_map_info = wavelet_map_info if wavelet_map_info is not None else map_info
        wavelet = wavelet_scalar_at_coords(np.array(ref_coords), lookup_map_info, wavelet_map)
        mult = min(
            1.0 + parameter.WAVELET_DTH_ALPHA * (1.0 - wavelet),
            parameter.WAVELET_DTH_MAX_MULT,
        )
        return base * mult
                    ...
                    if first_node:
                        ...
                        if coords not in self.nodes_dict.find(ref).data.neighbor_set and (
                            ...
                            np.array(coords) - np.array(ref)) > (
                                        self.adaptive_distance_threshold(ref, map_info, wavelet_map, wavelet_map_info))):
        ...
        for center_node, dstar in self.cluster_center_node_dict.values():
            ...
            for i, coords in enumerate(path):
                if first_node:
                    if coords not in self.nodes_dict.find(ref).data.neighbor_set and (
                            check_collision(np.array(ref), np.array(coords), map_info) or np.linalg.norm(
                        np.array(coords) - np.array(ref)) > self.adaptive_distance_threshold(
                            ref, map_info, wavelet_map, wavelet_map_info
                        )):
                        assert i >= 1, print(self.nodes_dict.find(ref).data.neighbor_set, path, dist)
                        key_node_coords = path[i - 1]
                        if (key_node_coords[0], key_node_coords[1]) in self.key_node_dict.keys():
    ...
    def update_clustered_nodes(self, center_node, clustered_non_zero_utility_nodes):
        ...
        for neighbor_coords in center_node.neighbor_set:
            if neighbor_coords == (center_node.coords[0], center_node.coords[1]):
                continue
            neighbor_node_wrapper = self.nodes_dict.find(neighbor_coords)
            if neighbor_node_wrapper is None:
                continue
            neighbor_node = neighbor_node_wrapper.data
            ...
            for node in nodes_to_check:
                for coords in node.neighbor_set:
                    if coords == (node.coords[0], node.coords[1]):
                        continue
                    neighbor_node_wrapper = self.nodes_dict.find(coords)
                    if neighbor_node_wrapper is None:
                        continue
                    neighbor_node = neighbor_node_wrapper.data
